...
namespace SuperBMD.Materials
{
    ...
    public class BinaryTextureImage
    {
        ...
        private sealed class Palette
        {
            ...
            public void Load(ref EndianBinaryReader reader, uint paletteEntryCount)
            {
                ...
                paletteData = reader.ReadBytes((int)paletteEntryCount * 2);
            }
        ...
        public void Load(ref EndianBinaryReader stream, int headerStart, int imageIndex = 0)
        {
            ...
            PaletteCount = stream.ReadUShort();
            int paletteDataOffset = stream.ReadInt();
            //EmbeddedPaletteOffset = stream.ReadInt();
            MipMap = stream.ReadByte();
            EdgeLOD = stream.ReadBool();
            BiasClamp = stream.ReadBool();
            ...
            stream.Skip(); //LonelyPaddingByte = stream.ReadByte();
            LODBias = ((float)stream.ReadShort()) * (1 / 100.0f);

            int imageDataOffset = stream.ReadInt();

            // Load the Palette data 
            stream.Position = headerStart + paletteDataOffset + (0x20 * imageIndex);
            imagePalette = new Palette();
            imagePalette.Load(ref stream, PaletteCount);

            // Now load and decode image data into an ARGB array.
            stream.Position = headerStart + imageDataOffset + (0x20 * imageIndex);
            ...
            {
                m_rgbaImageData = DecodeData(ref stream, Width, Height, Format, imagePalette, PaletteFormat);
                m_rgbaMipImageData = new List<byte[]>();
                if (ImageCount > 1)
                {
                    for (ushort i = 1; i < ImageCount; i++)
                    {
                        ...
                        byte[] mipdata = DecodeData(ref stream, mipWidth, mipHeight, Format, imagePalette, PaletteFormat);
        ...
        public void WriteHeader(ref EndianBinaryWriter writer)
        {
        ...
        #region Decoding
        private static byte[] DecodeData(ref EndianBinaryReader stream, uint width, uint height, TextureFormats format, Palette imagePalette, PaletteFormats paletteFormat)
        {
            switch (format)
            {
                case TextureFormats.I4:
                    return DecodeI4(ref stream, width, height);
                case TextureFormats.I8:
                    return DecodeI8(ref stream, width, height);
                case TextureFormats.IA4:
                    return DecodeIA4(ref stream, width, height);
                case TextureFormats.IA8:
                    return DecodeIA8(ref stream, width, height);
                case TextureFormats.RGB565:
                    return DecodeRgb565(ref stream, width, height);
                case TextureFormats.RGB5A3:
                    return DecodeRgb5A3(ref stream, width, height);
                case TextureFormats.RGBA32:
                    return DecodeRgba32(ref stream, width, height);
                case TextureFormats.C4:
                    return DecodeC4(ref stream, width, height, imagePalette, paletteFormat);
                case TextureFormats.C8:
                    return DecodeC8(ref stream, width, height, imagePalette, paletteFormat);
                case TextureFormats.CMPR:
                    return DecodeCmpr(ref stream, width, height);
        ...
        private static byte[] DecodeRgba32(ref EndianBinaryReader stream, uint width, uint height)
        {
        ...
        private static byte[] DecodeC4(ref EndianBinaryReader stream, uint width, uint height, Palette imagePalette, PaletteFormats paletteFormat)
        {
        ...
        private static byte[] DecodeC8(ref EndianBinaryReader stream, uint width, uint height, Palette imagePalette, PaletteFormats paletteFormat)
        {
        ...
        private static byte[] DecodeRgb565(ref EndianBinaryReader stream, uint width, uint height)
        {
                ...
                for (int xBlock = 0; xBlock < numBlocksW; xBlock++)
                {
                    ...
                    for (int pY = 0; pY < 4; pY++)
                    {
                        for (int pX = 0; pX < 4; pX++)
                        {
                            ...
                            ushort sourcePixel = stream.ReadUShort();
        ...
        private static byte[] DecodeCmpr(ref EndianBinaryReader stream, uint width, uint height)
        {
                ...
                for (int xBlock = 0; xBlock < numBlocksW; xBlock++)
                {
                    for (int ySubBlock = 0; ySubBlock < 2; ySubBlock++)
                    {
                        for (int xSubBlock = 0; xSubBlock < 2; xSubBlock++)
                        {
                            ...
                            byte[] subBlockData = DecodeCmprSubBlock(ref stream);
        ...
        private static byte[] DecodeCmprSubBlock(ref EndianBinaryReader stream)
        {
            byte[] decodedData = new byte[4 * 4 * 4];

            ushort color1 = stream.ReadUShort();
            ushort color2 = stream.ReadUShort();
        ...
        private static byte[] DecodeIA8(ref EndianBinaryReader stream, uint width, uint height)
        {
            uint numBlocksW = (width + 3) / 4; //4 byte block width
            uint numBlocksH = (height + 3) / 4; //4 byte block height 
        ...
        private static byte[] DecodeIA4(ref EndianBinaryReader stream, uint width, uint height)
        {
            uint numBlocksW = (width + 7) / 8;
            uint numBlocksH = (height + 3) / 4;
        ...
        private static byte[] DecodeI4(ref EndianBinaryReader stream, uint width, uint height)
        {
        ...
        private static byte[] DecodeI8(ref EndianBinaryReader stream, uint width, uint height)
        {
        ...
        private static byte[] DecodeRgb5A3(ref EndianBinaryReader stream, uint width, uint height)
        {
            uint numBlocksW = (width + 3) / 4; //4 byte block width
            uint numBlocksH = (height + 3) / 4; //4 byte block height 
