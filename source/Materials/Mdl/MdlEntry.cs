...
namespace SuperBMD.Materials.Mdl
{
    public class MdlEntry
    {
        ...
        private void GenerateTevStageCommands(Material mat)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                for (int j = 0; j < 2; j++)
                {
                    ...
                    if (mat.TevOrders[tevOrderIndex] is null)
                    {
        ...
        private void GenerateTevColorCommands(Material mat)
        {
            ...
            {
                int colorIndex = i - 1;
                if (mat.TevColors[colorIndex] is null)
                    continue;
        ...
        private void GenerateBlendModeCommands(Material mat)
        {
            ...
            switch (mat.BlendMode.Type)
            {
                ...
                case BlendModeType.Subtract:
                    ...
                    break;
                case BlendModeType.Logic:
                    pe_cmode0.SetFlag(true, 1);
                    break;
            }

            pe_cmode0.SetBits((int)mat.BlendMode.Operation, 12, 4);
            pe_cmode0.SetBits((int)mat.BlendMode.DestinationFact, 5, 3);
            pe_cmode0.SetBits((int)mat.BlendMode.SourceFact, 8, 3);
        ...
        private void GenerateChannelControlCommands(Material mat)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if (mat.ChannelControls[channelControlIndex] is null)
                {
